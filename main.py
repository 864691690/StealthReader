...
# ================= 本地章节目录 =================
class LocalChapterSelector(QDialog):
    """本地 TXT 章节目录对话框"""
    CHAPTER_PATTERNS = [
        # 中文数字章节
        re.compile(r'^\s*第[一二三四五六七八九十百千万零\d]+[章节回卷集部篇]\s*.+', re.MULTILINE),
        # 纯数字章节
        re.compile(r'^\s*第\d+[章节回卷集部篇]\s*.+', re.MULTILINE),
        # 英文 Chapter
        re.compile(r'^\s*Chapter\s+\d+\s*.+', re.MULTILINE | re.IGNORECASE),
        # 卷/册/篇（无"第"）
        re.compile(r'^\s*[卷册篇]\s*[一二三四五六七八九十百千万零\d]+\s*.+', re.MULTILINE),
        # Part/Book/Volume
        re.compile(r'^\s*(Part|Book|Volume)\s+\d+\s*.+', re.MULTILINE | re.IGNORECASE),
    ]

    def __init__(self, full_text, current_pos, parent=None):
        super().__init__(parent)
        self.main_window = parent
        ...
        # 检测章节
        self.chapters = self.detect_chapters(full_text)
        if not self.chapters:
            # 无章节 → 按3000字分段
            chunk = 3000
            for i in range(0, len(full_text), chunk):
                title = f"段 {i // chunk + 1} (pos {i})"
                self.chapters.append((i, title))

        self.init_ui()

    @staticmethod
    def detect_chapters(text):
        """检测章节标题及位置，返回 [(start_pos, title), ...]"""
        chapters = []
        for pat in LocalChapterSelector.CHAPTER_PATTERNS:
            for m in pat.finditer(text):
                start = m.start()
                line = m.group().strip()
                chapters.append((start, line))
        # 去重并按位置排序
        seen = set()
        deduped = []
        for pos, title in sorted(chapters, key=lambda x: x[0]):
            if pos not in seen:
                seen.add(pos)
                deduped.append((pos, title))
        return deduped

    def find_current_chapter_idx(self):
        """用 <= 找包含 current_pos 的章节（修正 closest_idx bug）"""
        idx = 0
        for i, (pos, _) in enumerate(self.chapters):
    ...
    def init_ui(self):
        self.setWindowTitle(f"📖 目录 (本地 · 共 {len(self.chapters)} 章)")
        ...
        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText("搜索章节标题…")
        ...
        # 状态标签
        self.status_label = QLabel(f"共 {len(self.chapters)} 章")
        layout.addWidget(self.status_label)
        ...
        # 填充列表
        self.all_items = []
        current_idx = self.find_current_chapter_idx()
        ...
        # 高亮当前章节
        if current_idx < self.list_widget.count():
            self.list_widget.setCurrentRow(current_idx)
            self.list_widget.scrollToItem(self.list_widget.currentItem(), QAbstractItemView.PositionAtCenter)
    ...
    def filter_chapters(self, keyword):
        ...
        self.status_label.setText(f"显示 {self.list_widget.count()} / {len(self.chapters)} 章")

    def on_item_double_clicked(self, item):
        self.selected_pos = item.data(Qt.UserRole)
...
class StealthReader(QWidget):
    ...
    def calc_prev_page_start(self):
        ...
        try:
            ...
            if not prev_content:
                return max(0, self.local_start_index - 1000)

            self.text_edit.setPlainText(prev_content)

            scrollbar = self.text_edit.verticalScrollBar()
            max_sb = scrollbar.maximum()

            if max_sb == 0:
                # prev_content 不足一屏 → 上一页起点就是 temp_start
                return temp_start

            scrollbar.setValue(max_sb)

            cursor = self.text_edit.cursorForPosition(QPoint(0, 0))
            pos_in_prev = cursor.position()
            if pos_in_prev < 0 or pos_in_prev > len(prev_content):
                pos_in_prev = 0

            chars_in_prev_page = len(prev_content) - pos_in_prev
            real_prev_start = self.local_start_index - chars_in_prev_page
            return max(0, min(real_prev_start, self.local_start_index - 1))
        finally:
    ...
    def open_local_toc_selector(self):
        """打开本地 TXT 章节目录"""
        if not self.is_local_mode or not self.local_full_text:
        ...
        toc = LocalChapterSelector(self.local_full_text, self.local_start_index, self)
        if toc.exec_() == QDialog.Accepted:
            self.local_start_index = toc.selected_pos
            self.local_page_history.clear()
            self.render_local_page()
            self.update_text_signal.emit(f"跳转到: {toc.selected_title}", False)
    ...
    def contextMenuEvent(self, event):
        ...
        cmenu.addAction("📖 章节目录 (本地)").triggered.connect(self.open_local_toc_selector)
        cmenu.addSeparator()
